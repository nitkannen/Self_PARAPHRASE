...
def translator(text):	
	...
	for word in words:
		...
		if word.upper() in slangs:
			words[j] = slangs[word.upper()].lower()
		j = j + 1
...
def clean_tweet(tweet):
	...
	# text = re.sub(r'\s\s+', r' ', text) # This line works..But the line above is simpler.
	...
	# Replace emoji patterns with a space
	...
	if text == "":
		print('Text becomes null. Re-processing the text..')
		text = tweet.strip()
