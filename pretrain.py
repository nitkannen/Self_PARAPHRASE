...
class ABSADataset(Dataset):
    ...
    def __init__(self, data_samples, tokenizer):
        ...
        with self.tokenizer.as_target_tokenizer():
          self.t5_decoder_input = [self.tokenizer(data_samples[i]['masked_target']) for i in range(len(data_samples) )]
        ...
        self.t5_decoder_input_labels = [self.t5_decoder_input[i]['input_ids'] for i in range(len(data_samples))]
        self.t5_decoder_attention_mask = [self.t5_decoder_input[i]['attention_mask'] for i in range(len(data_samples))]
...
def collate_fn(batch):
    ...
    decoder_attention_mask = pad_sequence([torch.tensor(dec_att) for dec_att in (decoder_attention_mask)], batch_first = True)
    labels = torch.tensor(labels)
  
  

    return (input_ids, attention_mask,decoder_input_labels, decoder_attention_mask, labels)
...
if __name__ == '__main__':
  ...
  parser = argparse.ArgumentParser()
    # basic settings
  parser.add_argument("--pretrain_datapath", default='15res', type=str, required=True,
  ...
  loader = DataLoader(
                      Contrastive_Dataset, collate_fn = collate_fn ,shuffle = False,  batch_size=
                      args.batch_size
            )
  ...
  for epoch in range(epochs):
    ...
    if(epoch == 10 or epoch == 8 or epoch == 4 or epoch == 6 or epoch == 12 or epoch == 3 or epoch == 14): ## needs editing
